...
export default function MealFeedbackPage() {
  ...
  const loadData = async () => {
    ...
    if (!goalRes.data || !goalRes.data.is_premium) { router.push('/'); return }
    if (!mealsRes.data || mealsRes.data.length === 0) { router.push('/'); return }
    ...
    await fetchInitialFeedback(mealsRes.data, goalRes.data, total)
  }

  const fetchInitialFeedback = async (mealsData: any[], goalData: any, totalData: any) => {
    setFeedbackStatus('loading')
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) return

    try {
      const res = await fetch('/api/meal-feedback-group', {
        method: 'POST',
  ...
  const mealTotal = meals.reduce((acc, m) => ({
        ...
        <div className="bg-white rounded-2xl p-4 border border-[#DDD6C8] mb-4">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs text-[#8A8377]">{MEAL_LABELS[mealType] || mealType} · {today}</div>
              <div className="text-sm font-semibold text-[#2C2A26]">{meals.length}品</div>
              <div className="text-xs text-[#8A8377] mt-0.5">
                合計 {Math.round(mealTotal.calories)}kcal · P{Math.round(mealTotal.protein)}g · F{Math.round(mealTotal.fat)}g · C{Math.round(mealTotal.carbs)}g
              </div>
            </div>
          </div>
          <div className="flex flex-col">
            {meals.map((m) => (
              <div key={m.id} className="flex items-center justify-between py-1.5 border-b border-[#EFE8DA] last:border-0">
                <span className="text-sm text-[#2C2A26] truncate flex-1 mr-2">{m.food_name}</span>
                <span className="text-xs text-[#8A8377] flex-shrink-0">{m.calories}kcal</span>
              </div>
            ))}
          </div>
        </div>
